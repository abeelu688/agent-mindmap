...
import { LlmProviderError } from "./llmError";
import type {
  MergedOutline,
  MergedOutlineDetail,
  MergedOutlineNode,
...
function pickString(obj: Record<string, unknown>, key: string, maxLen: number): string | undefined {
  const v = obj[key];
  if (typeof v !== "string") {
    return undefined;
  }
...
function parseOutlineDetail(value: unknown): OutlineDetail | undefined {
  ...
  const clipped = text.length > MAX_DETAIL_TEXT ? text.slice(0, MAX_DETAIL_TEXT - 3) + "..." : text;
  return {
    text: clipped,
...
function parseOutlineNode(value: unknown, depth: number): OutlineNode | undefined {
  ...
  const normalizedDetails = children.length > 0 ? undefined : details.length ? details : undefined;
...
function parseMergedDetail(value: unknown): MergedOutlineDetail | undefined {
  ...
  for (const s of sourcesRaw) {
    ...
    const turnIndex =
      typeof so.turnIndex === "number" && Number.isInteger(so.turnIndex) && so.turnIndex >= 0
        ? so.turnIndex
        : undefined;
...
function parseMergedOutlineNode(value: unknown, depth: number): MergedOutlineNode | undefined {
  if (!value || typeof value !== "object" || depth > MAX_DEPTH) {
    return undefined;
  }
  const obj = value as Record<string, unknown>;
