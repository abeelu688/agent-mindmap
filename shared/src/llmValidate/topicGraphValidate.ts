...
import type { TopicGraph } from "../storeTypes";
...
/** Trie merge key: fold hyphen/underscore variants (e.g. foo-bar → foobar). */
...
export function validateTopicGraph(value: unknown): TopicGraph {
  ...
  for (const t of topics) {
    ...
    for (const it of itemsRaw) {
      ...
      const sourceTurnIndicesRaw = (it as Record<string, unknown>).sourceTurnIndices;
