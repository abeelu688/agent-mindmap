...
import { __testingLlmIoDump } from "@agent-mindmap/core";
import {
  dumpDirForWorkspace,
  LLM_DUMP_FOLDER,
  dumpLlmReplay,
...
// Test-specific LlmDumpDeps — dump is always enabled, roots are controlled per-test
const testDeps: LlmDumpDeps = {
  isDumpEnabled: () => true,
  resolveDumpRoots: () => [],
  logInfo: () => {},
  logWarn: () => {},
...
describe("llmIoDump", () => {
  it("dumpDirForWorkspace joins workspace root and folder name", () => {
    expect(dumpDirForWorkspace("/home/proj")).toBe(`/home/proj/${LLM_DUMP_FOLDER}`);
  });
  ...
  it("writeLlmIoDump writes expected files under stage subfolder", async () => {
    ...
    expect(JSON.parse(readFileSync(join(runDir, "parsed.json"), "utf8"))).toEqual({
      domains: [],
      nodes: [],
    });
  });
  ...
  it("dumpLlmReplay sets source in meta.json", async () => {
    ...
    const runDir = join(root, "session-analysis", readdirSync(join(root, "session-analysis"))[0]!);
    const meta = JSON.parse(readFileSync(join(runDir, "meta.json"), "utf8")) as {
      source: string;
      attempt: number;
    };
  ...
  it("compactTimestamp is filesystem-safe", () => {
    const ts = __testingLlmIoDump.compactTimestamp(new Date("2026-06-02T14:30:52.123Z"));
