...
describe("scaleReattachTimeoutMs", () => {
  ...
  it("keeps base for small chain counts", () => {
    expect(scaleReattachTimeoutMs(90_000, REATTACH_TIMEOUT_CHAINS_PER_SLOT)).toBe(
      90_000
    );
  });
...
describe("scaleMergeSessionAnalysisTimeoutMs", () => {
  it("scales batch 2 delta snapshot+batch (6 sessions) to 270s at 90s base", () => {
    expect(
      scaleMergeSessionAnalysisTimeoutMs(90_000, 6, { mergeMode: "delta" })
    ).toBe(270_000);
  });
  ...
  it("batch 1 full (5 sessions) stays at 180s", () => {
    expect(scaleMergeSessionAnalysisTimeoutMs(90_000, 5, { mergeMode: "full" })).toBe(
      180_000
    );
  });
});
