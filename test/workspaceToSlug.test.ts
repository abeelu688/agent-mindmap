...
describe("workspaceToSlug", () => {
  ...
  it("encodes unix absolute paths", () => {
    ...
    expect(workspaceToSlug("/home/welde/cursor/airecorder")).toBe(
...
describe("slugToWorkspacePath", () => {
  ...
  it("decodes unix slugs", () => {
    vi.stubGlobal("process", { ...process, platform: "linux" });
    expect(slugToWorkspacePath("home-welde-proj")).toBe("/home/welde/proj");
  });

  it("decodes windows slugs with drive letter", () => {
    vi.stubGlobal("process", { ...process, platform: "win32" });
    expect(slugToWorkspacePath("d-cursor-projects-agent-mindmap")).toBe(
      "D:\\cursor_projects\\agent-mindmap"
    );
  });
});
