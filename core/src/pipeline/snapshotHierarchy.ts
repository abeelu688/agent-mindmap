/**
 * Multi-level snapshot hierarchy: L1 leaf merges, promotion, root rebuild + M3.
 *
 * Moved from extension/src/pipeline/snapshotHierarchy.ts in R2.
 * Uses Store injection, ProgressReporter, sanitizeRecord and LlmDumpDeps ports.
 * tryReuseBatchMerge is injected as a callback since it remains extension-local.
 */
import {
...
/**
 * Callback type for tryReuseBatchMerge — remains in extension (batchMergeCache).
 * When not provided, the cache-reuse logic is skipped.
 */
export type TryReuseBatchMergeFn = (opts: {
  storeDir: string;
  projectSlug: string;
 ...
 * transcript parser. When not provided, records are passed through as-is.
 */
export type SanitizeRecordFn = (record: SessionRecord) => Promise<SessionRecord>;
...
export type SnapshotHierarchyLlmOpts = {
  ...
  /** Batch merge cache lookup — extension injects from batchMergeCache. */
  tryReuseBatchMergeFn?: TryReuseBatchMergeFn;
};
...
export async function runLeafSnapshotMerge(
  ...
  let segmentEquivalences: SegmentEquivalence[] = [];
...
export async function runBatchSnapshotPipeline(
  ...
  if (opts.tryReuseBatchMergeFn) {
    const cached = await opts.tryReuseBatchMergeFn({
    ...
    getCoreLogger().info(`[runBatchSnapshotPipeline] cache miss: ${cached.reason}`);
  }
  ...
  await opts.store.writeConceptTrieMerge(merge);
  return merge;
}
