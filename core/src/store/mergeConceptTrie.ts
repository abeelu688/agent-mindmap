...
export type ConceptMergeOptions = {
  ...
  /**
   * Prep function for records before trie build. When `ontologyForPrep` is set
   * and `recordsAlreadyPrepared` is false, this is called to apply ontology
   * transforms. Extension passes `prepareRecordsForFinalTrie`; CLI passes
   * its own adapter.
   */
  prepareRecordsFn?: (
...
function conceptTrieEmptyLeaf(
  ...
  if (filteredRecordCount === 0 || totalTopics === 0) {
    // l10n: "(No analyzed sessions in the library; ...)" / "（库中暂无已分析的会话；...）"
    return leaf(
      "(No analyzed sessions in the library; run Open Latest Session or Analyze Project Sessions first.)"
    );
  }
  if (rootHiddenTopics > 0) {
    // l10n: "(Some topics have invalid conceptPath; ...)" / "（部分主题的 conceptPath 无效；...）"
    return leaf("(Some topics have invalid conceptPath; refresh a session to re-analyze.)");
  }
  // l10n: "(No topics with conceptPath in the library; ...)" / "（库中暂无含 conceptPath 的主题；...）"
  return leaf("(No topics with conceptPath in the library; refresh a session to re-analyze.)");
}
...
/**
 * Build a concept merge record from sanitized records.
 * The `sanitizeFn` callback is responsible for sanitizing each record.
 */
export async function buildConceptMergeRecordAsync(
  records: SessionRecord[],
  sanitizeFn: (record: SessionRecord) => Promise<SessionRecord>,
  options: ConceptMergeOptions = {}
): Promise<MergeRecord> {
  const sanitized = await Promise.all(records.map((r) => sanitizeFn(r)));
...
function recordsForTrieBuild(
  ...
  if (options.prepareRecordsFn) {
    return options.prepareRecordsFn(
      records,
  ...
  // No prep function — return records as-is (caller should have prepped them).
  return records;
}
