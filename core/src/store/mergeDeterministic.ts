...
export type DeterministicMergeOptions = {
  /** Override the root node label. Defaults to "Agent Mind Map · All". */
  title?: string;
...
export function buildDeterministicMergeMindMap(
  ...
  const title =
    ...
    (options.projectSlug
      ...
      : // l10n: "Agent Mind Map · All" / "Agent Mind Map · 全部"
        "Agent Mind Map · All");
  ...
  if (!projectNodes.length) {
    return {
      ...
      children: [
        ...
        leaf("(No analyzed sessions in the library)"),
      ],
    };
  }
...
/**
 * Build a deterministic merge record from sanitized records.
 * The `sanitizeFn` callback is responsible for sanitizing each record
 * (e.g. resolving user query counts, fixing outlines). In the extension
 * this calls `sanitizeSessionRecord`; in the CLI it uses the core version.
 */
