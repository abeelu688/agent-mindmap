/**
 * LLM IO dump — writes LLM prompt/response pairs to disk for debugging.
 *
 * Moved from extension/src/llm/llmIoDump.ts in P1.10.
 * All VS Code config reads and logging are delegated through `LlmDumpDeps`.
 */
...
/** Resolve dump roots using the provided deps. */
export function resolveLlmDumpRoots(deps: LlmDumpDeps): string[] {
  const enabled = deps.isDumpEnabled();
  if (!enabled) {
    agentDebugLog(
  ...
  const roots = deps.resolveDumpRoots();
  agentDebugLog("llmIoDump.ts:resolveLlmDumpRoots", "resolved dump roots", { roots }, "A");
  return roots;
}

/** @deprecated Prefer resolveLlmDumpRoots; first root or undefined when disabled. */
export function resolveLlmDumpDir(deps: LlmDumpDeps): string | undefined {
  const roots = resolveLlmDumpRoots(deps);
  return roots[0];
}

export function logLlmDumpLocationsOnce(deps: LlmDumpDeps): void {
  if (loggedDumpRoots || !deps.isDumpEnabled()) {
    return;
  }
  ...
  deps.logInfo(`LLM dumps enabled → ${roots.join(" | ")}`);
}
...
export async function writeLlmIoDump(
  ...
  const roots = payload.dumpRoot ? [payload.dumpRoot] : resolveLlmDumpRoots(deps);
  agentDebugLog(
  ...
  for (const root of roots) {
    try {
      ...
      deps.logInfo(`LLM dump (${payload.source ?? "live-cli"}): ${outDir}`);
      agentDebugLog(
        "llmIoDump.ts:writeLlmIoDump",
        "dump write ok",
      ...
      deps.logWarn(`LLM dump failed (${root})`, { error: String(err) });
      agentDebugLog(
        "llmIoDump.ts:writeLlmIoDump",
        "dump write failed",
        {
...
export async function dumpLlmReplay(
  opts: {
    stageId: string;
    responseSchema: LlmResponseSchema;
    providerId: string;
  ...
  if (!opts.dumpRoot && !deps.isDumpEnabled()) {
    agentDebugLog(
...
export async function dumpLlmCallResult(
  opts: {
    input: {
      prompt: string;
      model?: string;
...
export const __testingLlmIoDump = {
  compactTimestamp,
  randomSuffix,
  dumpDirForWorkspace,
  resolveLlmDumpRoots,
