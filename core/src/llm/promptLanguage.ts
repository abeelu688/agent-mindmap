...
/**
 * Pluggable accessor for the user's `agentMindmap.llm.promptLanguage` setting.
 * Extension wires this to vscode config; CLI / tests inject directly.
 */
...
function readSetting(): PromptLanguageSetting {
  const raw = promptLanguageSettingProvider();
  if (raw === "zh" || raw === "en") {
    return raw;
  }
  return "auto";
...
export const __testingPromptLanguage = {
