...
import { sortReattachStepsForApply, reattachStepsToMoves } from "./reattachStepsToMoves";
...
import type { ReattachMove, ReattachStep } from "./types";
import type { SegmentEquivalence } from "./types";
import type { SessionRecord } from "../store/storeTypes";

// Re-export from the existing module to avoid duplication.
export {
  reattachStepToMove,
  reattachStepsToMoves,
  sortReattachStepsForApply,
} from "./reattachStepsToMoves";
...
export function applyReattachStepsToRecords(
  ...
  if (nodeCatalog) {
    ...
    if (!resolved.length) {
      getCoreLogger().info(
        "[reattachSteps] all steps failed nodeCatalog resolution; paths unchanged"
      );
      return records;
    }
    if (resolved.length < steps.length) {
      getCoreLogger().info(
        `[reattachSteps] ${steps.length - resolved.length} step(s) dropped after catalog resolution`
      );
    }
  }
