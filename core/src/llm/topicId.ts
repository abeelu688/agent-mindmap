 ...
 */
export function topicIdForTopic(sessionId: string, topic: Pick<Topic, "title" | "items">): string {
  const items = (topic.items ?? []).map((i) => i.text).slice(0, 8);
  return sha256Hex(JSON.stringify({ sessionId, title: topic.title, items })).slice(0, 24);
}
