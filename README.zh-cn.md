...
完整路线图请参阅 [docs/IMPROVEMENT-PLAN.md](docs/IMPROVEMENT-PLAN.md)。重点：

- [ ] 统一错误处理系统
- [ ] 入口文件重构（commands/ + batch/ 拆分）
- [ ] GitHub Actions CI（lint、类型检查、测试）
- [ ] LLM Prompt 国际化（目前仅中文）
- [ ] 更多 UI 语言支持（ja、ko 等）
- [ ] 架构文档

## 隐私

对话记录可能包含本地文件路径和代码片段。扩展仅将对话内容发送到已配置的 CLI（`cursor-agent` 或 `claude`），由其在现有订阅条款下转发。库中仅存储摘要化的 `TopicGraph` 和元数据——**不包含**原始对话。
